...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildMetadata({
    ...
    canonical: `${SITE_URL}/${lang}/fahrzeuge/${vehicleSlug}`,
    ogImage: imageUrl,
    ogType: "website",
    lang,
...
export default async function VehicleDetailPageDe({ params }: Props) {
  const { lang, vehicleSlug } = await params;
  ...
  const jsonLd = wrapInGraph(
    buildVehicleProduct({
      ...
      url: `${SITE_URL}/${lang}/fahrzeuge/${vehicleSlug}`,
    }),
  );
  ...
  return (
    <>
      ...
      <VehicleDetailView
        vehicle={vehicle}
        lang={lang}
        vehiclesSegment="fahrzeuge"
        brandsSegment="marken"
      />
    </>
  );
}
